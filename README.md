...
## Executive Summary
This project is a high-performance, non-blocking asynchronous security and user routing engine. It is designed to handle concurrent connections efficiently, implement cryptographic token lifecycles, and utilize fast in-memory layers to prevent unauthorized usage without slowing down requests. The core boundary of this project focuses purely on execution and runtime optimization.

## Core Features
* **Asynchronous Execution Model:** Built entirely on a non-blocking I/O single-thread loop worker model using FastAPI and `asyncpg`.
...
* **Stateful Token Invalidation (Zero-Trust):** Utilizes a fast memory layer (Redis) to track logouts. When a user logs out, their active token is blacklisted instantly for its remaining valid lifespan.
